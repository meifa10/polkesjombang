...
<body class="bg-slate-100 min-h-screen font-sans antialiased">
    ...
    {{-- LOGIKA PERHITUNGAN DINAMIS (Agar tampilan & hitungan selalu sinkron) --}}
    @php
        $biayaDokterBaru = 10000;
        $biayaAdmin = $pembayaran->biaya_admin ?? 10000;
        $biayaObat = $pembayaran->total_obat ?? 0;
        $totalTagihanFix = $biayaDokterBaru + $biayaAdmin + $biayaObat;
    @endphp
                ...
                <div class="p-8">
                    ...
                    <div class="bg-slate-50 rounded-3xl p-8 border border-slate-200 mb-6">
                        ...
                        <h2 class="text-5xl font-black text-slate-900 leading-none">
                            <span class="text-2xl text-emerald-600">Rp</span> {{ number_format($totalTagihanFix, 0, ',', '.') }}
                        </h2>
                    </div>

                    {{-- DETAIL --}}
                    <div class="space-y-4 mb-8">
                        <div class="flex justify-between items-center text-slate-500"><span class="font-medium">Pasien</span><span class="font-black text-slate-800 uppercase">{{ $pembayaran->pendaftaran->nama_pasien ?? '-' }}</span></div>
                    ...
                    @else
                        <a href="/pembayaran/struk/{{ $pembayaran->id }}" target="_blank" class="w-full h-16 rounded-2xl flex items-center justify-center bg-emerald-600 text-white font-black uppercase tracking-[0.1em]">
    ...
    <script>
        ...
        if (payBtn) {
            payBtn.addEventListener('click', function () {
                snap.pay(snapToken, { onSuccess: () => window.location.reload() });
            });
        }
    </script>
</body>
